...
public class GuestbookListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {	
		...
		//DB
		List<GuestbookDTO> list = GuestbookDAO.getinstance().getList(startNum, endNum);
		// 부모클래스는 모든 자식클래스를 참조하기 때문에, 부모 인터페이스 List 가 자식 클래스 ArrayList를 참조할 수 있다. 
		...
		if(list != null) {
			for (GuestbookDTO guestbookDTO : list) {			
				...
				out.println("<td colspan = '4'><pre style ='overflow: auto; white-space:pre-line; word-break: break-all; width: 480px; height: 100px;'>"+guestbookDTO.getContent()+"</pre></td>");
				out.println("</table>");
				out.println("<br>");
				out.println("<hr width = 500 align = 'left' color= red>");
			}
			for(int i = 1; i <= totPage ; i++) {
				if(i == pg) out.println("<a id ='currentPaging' href = '/guestbookservlet/GuestbookList?pg=" + i +"'>" + "[" + i + "]" + "</a>");
				else out.println("<a id ='paging' href = '/guestbookservlet/GuestbookList?pg=" + i +"'>" + "[" + i + "]" + "</a>");	
