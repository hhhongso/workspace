...
public class Board extends JPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) { // ActionListener 이벤트
		if (e.getSource() == btnWrite) { // 글쓰기 이벤트. Dialog에서 처리.
			...
			String text = "" + tableBoardList.getValueAt(rowIdx, 2);
			String writeTime = "" + tableBoardList.getValueAt(rowIdx, 3);
			...
			if (temp instanceof BoardDTO) {
				...
				if (boardDTO.getStatus() == Status.DELETE_BOARD) {
					boardDTO.setStatus(Status.GET_FROM_DB);
					main.request(boardDTO);
					while (true) {
						boardDTO = (BoardDTO) main.response();
						if (boardDTO.getStatus() == Status.GET_FROM_DB) {
